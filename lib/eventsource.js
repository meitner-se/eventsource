// Sep 25, 2013 - 7dff45f8aa78d4cf833f3c6afd22fc10698400cb
...
(function(global) {
    ...
    var isCORSSupported = Boolean(
        XHR && new XHR().withCredentials !== undefined
    );
    ...
    function EventSource(url, options) {
        ...
        var withCredentials = Boolean(
            isCORSSupported && options && options.withCredentials
        );
        var initialRetry = getDuration(options ? options.retry : NaN, 1000);
        var heartbeatTimeout = getDuration(
            options ? options.heartbeatTimeout : NaN,
            45000
        );
        var lastEventId =
            (options && options.lastEventId && String(options.lastEventId)) ||
            "";
        var that = this;
        ...
        function onProgress(isLoadEnd) {
            var responseText =
                currentState === OPEN || currentState === CONNECTING
                    ? xhr.responseText || ""
                    : "";
            var event = null;
            ...
            if (s !== "data:" && s !== "blob:") {
                s =
                    ...
                    ((url.indexOf("?", 0) === -1 ? "?" : "&") +
                        ...
                        "&r=" +
                        String(Math.random() + 1).slice(2));
            } else {
                s = url;
            }
            xhr.open(method, s, true);
